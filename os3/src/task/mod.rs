...
impl TaskManager {
    ...
    fn increase_syscall_time(&self,syscall_nummber:usize){
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].syscall_time[syscall_nummber]+=1;
    }
    ...
    fn get_task_info(&self, ti: *mut TaskInfo) {
        ...
        unsafe {
            ...
            (*ti).syscall_times = inner.tasks[current].syscall_time;
            (*ti).time = get_time_ms() - inner.tasks[current].time;
        }
    }
}
